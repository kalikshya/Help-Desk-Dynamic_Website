<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Track Problem Status</title>
<link rel="stylesheet" href="${pageContext.request.contextPath}/css/trackStatus.css">
</head>
<body>
<div class="dashboard-container">
    <!-- Sidebar -->
    <div class="sidebar">
       <h2><a href="${pageContext.request.contextPath}/userDashboard" class="dashboard-link">User Panel</a></h2>
        <ul>
            <li><a href="${pageContext.request.contextPath}/reportProblem">Report Problem</a></li>
            <li><a href="${pageContext.request.contextPath}/trackStatus">Track Status</a></li>
            <li><a href="${pageContext.request.contextPath}/submitFeedback">Submit Feedback</a></li>
            <li><a href="${pageContext.request.contextPath}/myProfile">My Profile</a></li>
            <li><a href="${pageContext.request.contextPath}/logout">Logout</a></li>
        </ul>
    </div>

    <!-- Main Content -->
    <div class="content">
        <h2>Track Your Problem</h2>

        <form action="#" method="post" class="track-form">
            <label for="problemId">Enter Problem ID:</label>
            <input type="text" id="problemId" name="problemId" placeholder="e.g., 117" required>
            <button type="submit">Check Status</button>
        </form>

        <!-- Example Result (can be updated dynamically) -->
        <div class="status-result">
            <h3>Status:</h3>
            <p class="status">In Progress</p>
        </div>
    </div>
</div>

<!-- Footer -->
<div class="footer">
    &copy; 2025 HelpDesk | Empowering Citizens
</div>

</body>
</html>